...
def get_curl_command(master_ip, slave_ip):
    command = "curl -X PUT \"http://admin:cdurq48YWLWtZtd@" + str(master_ip) + ":5986/_nodes/couchdb@" + str(
        slave_ip) + "\"" + " -d {}\n"
    return command
...
def get_crendential():
    return "\n[all:vars]\nansible_ssh_user=ubuntu\nansible_ssh_private_key_file=./ccc.pem\nansible_ssh_extra_arg" \
        ...
        for instance_name in instance_name_list:
            ...
            reservation = ec2_conn.run_instances(max_count=1,
                                                 image_id=jconfig['sys_types'][instance_name_list.index(instance_name)][
                                                     'image_id'],
                                                 placement=
                                                 jconfig['sys_types'][instance_name_list.index(instance_name)][
                                                     'placement'],
            ...
            logging.info("Create the instance: " + str(
                jconfig['sys_types'][instance_name_list.index(instance_name)]['name']) + " with instance id: "
                         + str(instance.id))
